...
ID_TOOLBAR_EMPENHO_CONTRATO_NOVO = 5001
ID_TOOLBAR_EMPENHO_CONTRATO_EDITAR = 5002
ID_TOOLBAR_EMPENHO_CONTRATO_EXCLUIR = 5003
ID_TOOLBAR_EMPENHO_CONTRATO_CRIAR_ARQUIVO = 5004


class WindowEmpenhoContrato(wx.MiniFrame):

    def __init__(self, parent):

        wx.MiniFrame.__init__(self, parent, id=wx.ID_ANY, size=(530, 300), pos=(300, 170), title=u"Informações de Empenho Contrato", style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
        self.panelEmpenhoContrato = wx.Panel(self, wx.ID_ANY)

        self.vbox1 = wx.BoxSizer(wx.VERTICAL)

        self.toolBar = wx.ToolBar(self, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.TB_TEXT)

        self.toolBar.AddLabelTool(ID_TOOLBAR_EMPENHO_CONTRATO_NOVO, "Novo", wx.Bitmap("./imagens/add.png"), shortHelp=u'Adiciona novo empenho')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_EMPENHO_CONTRATO_EDITAR, "Editar", wx.Bitmap("./imagens/edit.png"), shortHelp=u'Edita empenho selecionado')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_EMPENHO_CONTRATO_EXCLUIR, "Remover", wx.Bitmap("./imagens/remove.png"), shortHelp=u'Exclui empenho selecionado')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_EMPENHO_CONTRATO_CRIAR_ARQUIVO, "Gerar Arquivo", wx.Bitmap("./imagens/file.png"), shortHelp=u'Gera arquivo de empenho')
        self.toolBar.AddSeparator()
        self.toolBar.AddSeparator()
        ...
        self.cbCompetenciaForView = wx.ComboBox(self.panelEmpenhoContrato, -1, pos=(1, 5), size=(200, -1), choices=self.choicesCompetencias, style=wx.CB_READONLY)
        self.cbCompetenciaForView.Bind(wx.EVT_COMBOBOX, self.insereInCtrList)

        #ListCtrl
        self.hbox1 = wx.BoxSizer(wx.HORIZONTAL)
        self.empenhoListCtrl = wx.ListCtrl(self.panelEmpenhoContrato, wx.ID_ANY, pos=(0, 30), size=(525, 200), style=wx.LC_REPORT)
        self.empenhoListCtrl.InsertColumn(0, u'Número Contrato', width=150)
        ...
        #Binds
        self.Bind(wx.EVT_MENU, self.novoEmpenho, id=ID_TOOLBAR_EMPENHO_CONTRATO_NOVO)
        self.Bind(wx.EVT_MENU, lambda event: self.vizualizaEmpenho(event, self.idSelecionado), id=ID_TOOLBAR_EMPENHO_CONTRATO_EDITAR)
        self.Bind(wx.EVT_MENU, lambda event: self.excluiEmpenho(event, self.idSelecionado), id=ID_TOOLBAR_EMPENHO_CONTRATO_EXCLUIR)
        self.Bind(wx.EVT_MENU, self.geraArquivoWindow, id=ID_TOOLBAR_EMPENHO_CONTRATO_CRIAR_ARQUIVO)
    ...
    def toolBarControler(self, novo=True, editar=True, remover=True, gerar=True):
        ...
        self.toolBar.EnableTool(ID_TOOLBAR_EMPENHO_CONTRATO_NOVO, novo)
        self.toolBar.EnableTool(ID_TOOLBAR_EMPENHO_CONTRATO_EDITAR, editar)
        self.toolBar.EnableTool(ID_TOOLBAR_EMPENHO_CONTRATO_EXCLUIR, remover)
        self.toolBar.EnableTool(ID_TOOLBAR_EMPENHO_CONTRATO_CRIAR_ARQUIVO, gerar)
    ...
    def insereInCtrList(self, event):
        ...
        if self.cbCompetenciaForView.GetSelection() != -1:
            empenhos = ContratoEmpenho.query.filter_by(competencia=self.cbCompetenciaForView.GetValue()).all()

            for empenho in empenhos:

                index = self.empenhoListCtrl.InsertStringItem(sys.maxint, unicode(empenho.numeroContrato))
    ...
    def novoEmpenho(self, event):
        ...
        self.stNumeroContrato = wx.StaticText(self.panelNovoEmpenho, -1, u'Número Contrato', pos=(10, 70))
        self.cbNumeroContrato = wx.ComboBox(self.panelNovoEmpenho, -1, pos=(10, 90), size=(150, -1), choices=[], style=wx.CB_READONLY)
    ...
    def insereNumeroProcesso(self, event):
        ...
        self.cbNumeroContrato.Clear()

        contratos = Contrato.query.filter_by(competencia=self.cbCompetencia.GetValue()).all()

        if not contratos:
            self.message = wx.MessageDialog(None, u'Não existe contratos para a competência selecionada!', 'Info', wx.OK)
            self.message.ShowModal()
            self.cbNumeroContrato.Disable()

        else:
            for contrato in contratos:

                self.cbNumeroContrato.Append(unicode(contrato.numeroContrato))

            self.cbNumeroContrato.Enable()
    ...
    def valida(self):
        ...
        if self.cbNumeroContrato.GetSelection() == -1:

            self.message = wx.MessageDialog(None, u'Selecione uma opção no campo Número Contrato', 'Info', wx.OK)
            self.cbNumeroContrato.SetFocus()
            self.message.ShowModal()
            return 0
        ...
        empenhos = ContratoEmpenho.query.filter_by(numeroContrato=self.cbNumeroContrato.GetValue()).all()

        if empenhos:
            for empenho in empenhos:

                if empenho.numeroContrato == self.cbNumeroContrato.GetValue() and empenho.id != self.tcId.GetValue() and empenho.anoEmpenho == self.tcAnoEmpenho.GetValue() and empenho.notaEmpenho == self.tcNotaEmpenho.GetValue() and empenho.unidadeOrcamentaria == self.tcUnidadeOrcamentaria.GetValue():
    ...
    def salvarEmpenho(self, event):
        ...
        if self.valida():
            ...
            ContratoEmpenho(
                numeroContrato = unicode(self.cbNumeroContrato.GetValue()),
    ...
    def vizualizaEmpenho(self, event, idEmpenho):
        ...
        self.empenho = ContratoEmpenho.query.filter_by(id=idEmpenho).first()

        self.toolBarControler(False, False, False, False)

        self.windowVizualizaEmpenho = wx.MiniFrame(parent=self, id=wx.ID_ANY, size=(500, 270), pos=(300, 170), title=u'Editar - Empenho Contrato', style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
        ...
        self.stNumeroContrato = wx.StaticText(self.panelVizualizaEmpenho, -1, u'Número Contrato', pos=(10, 70))
        self.cbNumeroContrato = wx.ComboBox(self.panelVizualizaEmpenho, -1, pos=(10, 90), size=(150, -1), choices=[], style=wx.CB_READONLY)
        self.insereNumeroProcesso(None)
        self.cbNumeroContrato.SetValue(self.empenho.numeroContrato)
    ...
    def editarEmpenho(self, event):
        ...
        if self.valida():
            ...
            self.empenho.numeroContrato = unicode(self.cbNumeroContrato.GetValue())
    ...
    def excluiEmpenho(self, event, idEmpenho):
        ...
        remove_dial = wx.MessageDialog(None, u'Tem certeza que deseja excluir este empenho?', 'Excluir - Empenho Contrato', wx.YES_NO | wx.NO_DEFAULT | wx.ICON_QUESTION)
    ...
    def geraArquivoWindow(self, event):
        ...
        self.windowGeraArquivo = wx.MiniFrame(parent=self, id=wx.ID_ANY, size=(680, 470), pos=(300, 170), title=u"Gerar Arquivo de Empenho de Contrato", style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
        ...
        self.empenhoGeraArquivoListCtrl.InsertColumn(0, u'Num. Contrato', width=100)
        ...
        self.empenhoParaArquivoListCtrl.InsertColumn(0, u'Número Contrato', width=100)
        self.empenhoParaArquivoListCtrl.InsertColumn(1, u'Nota Empenho', width=80)
        self.empenhoParaArquivoListCtrl.InsertColumn(2, u'Unidade Orçamentária', width=70)
    ...
    def insereEmpenhoPorCompetencia(self, event):
        ...
        if self.competenciaAtual == unicode(self.cbGeraArquivoCompetencia.GetValue()):
            ...
            empenhos = ContratoEmpenho.query.filter_by(competencia=self.cbGeraArquivoCompetencia.GetValue()).all()
        else:

            empenhos = ContratoEmpenho.query.all()
            ...
            if len(empenhos) == self.empenhoParaArquivoListCtrl.GetItemCount():
                ...
                for empenho in empenhos:
                    ...
                    if self.empenhoParaArquivoListCtrl.GetItemCount() == 0:
                        index = self.empenhoGeraArquivoListCtrl.InsertStringItem(sys.maxint, unicode(empenho.numeroContrato))
                        ...
                        for x in range(self.empenhoParaArquivoListCtrl.GetItemCount()):
                            ...
                            if empenho.numeroContrato == unicode(self.empenhoParaArquivoListCtrl.GetItem(x, 0).GetText()) and empenho.notaEmpenho == unicode(self.empenhoParaArquivoListCtrl.GetItem(x, 1).GetText()) and empenho.unidadeOrcamentaria == unicode(self.empenhoParaArquivoListCtrl.GetItem(x, 2).GetText()) and empenho.anoEmpenho == unicode(self.empenhoParaArquivoListCtrl.GetItem(x, 3).GetText()):
                                igual = True
    ...
    def geraArquivoDialog(self, event):
        ...
        if self.empenhoParaArquivoListCtrl.GetItemCount() == 0:
            ...
            self.message = wx.MessageDialog(None, u'Selecione os empenhos para gerar o arquivo!', u'ATENÇÃO', wx.OK)
            self.message.ShowModal()
            return 0

        dlg = wx.FileDialog(self, message=u"Salvar ", defaultDir="", defaultFile="CONTRATOEMPENHO.REM", wildcard="Arquivo de Remessa (*.REM)|*.REM", style=wx.SAVE)
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if os.path.exists(self.path):
                ...
                if ret == wx.ID_YES:
                    ...
                    if self.geraArquivo():
                        self.message = wx.MessageDialog(None, u'Arquivo de Empenhos de Contrato gerado com sucesso!', 'Info', wx.OK)
                        self.message.ShowModal()
                ...
                if self.geraArquivo():
                    ...
                    self.message = wx.MessageDialog(None, u'Arquivo de Empenhos de Contrato com sucesso!', 'Info', wx.OK)
                    self.message.ShowModal()
    ...
    def geraArquivo(self):
        ...
        for x in range(self.empenhoParaArquivoListCtrl.GetItemCount()):
            ...
            try:
                ...
                empenho = ContratoEmpenho.query.filter_by(id=idEmpenho).first()

                f.write(unicode(empenho.numeroContrato.ljust(16).replace("'", "").replace("\"", "")))
