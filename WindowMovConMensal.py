...
class WindowMovConMensal(wx.MiniFrame):
    


    def __init__(self, parent):

        wx.MiniFrame.__init__(self, parent, id=wx.ID_ANY, size=(530, 340), pos=(300, 170), title=u"Movimentação Contábil Mensal", style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
        ...
        self.toolBar.AddLabelTool(ID_TOOLBAR_MOVCONINICIAL_NOVO, "Novo", wx.Bitmap("./imagens/add.png"), shortHelp=u'Adiciona novo Movimento Contábil Mensal')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_MOVCONINICIAL_EDITAR, "Editar", wx.Bitmap("./imagens/edit.png"), shortHelp=u'Edita Movimento Contábil Mensal selecionado')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_MOVCONINICIAL_EXCLUIR, "Remover", wx.Bitmap("./imagens/remove.png"), shortHelp=u'Exclui Movimento Contábil Mensal selecionado')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_MOVCONINICIAL_CRIAR_ARQUIVO, "Gerar Arquivo", wx.Bitmap("./imagens/file.png"), shortHelp=u'Gera arquivo de Movimento Contábil Mensal')
        self.toolBar.AddSeparator()
        self.toolBar.AddSeparator()
        ...
        self.Bind(wx.EVT_MENU, self.importaMovConMensal, id=ID_TOOLBAR_MOVCONINICIAL_IMPORTAR)
    ...
    def insereInCtrList(self, event):
        ...
        movimentos = MovConMensal.query.filter_by(competencia=self.cbCompetenciaForView.GetValue()).all()
    ...
    def novoMovConIni(self, event):
        ...
        self.windowNovoMovConIni = wx.MiniFrame(parent=self, id=wx.ID_ANY, size=(300, 280), pos=(300, 170), title=u'Novo - Movimetação Contábil Mensal', style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
    ...
    def salvarMovConIni(self, event):
        ...
        if self.valida():
            ...
            try:
                ...
                session.commit()
                self.message = wx.MessageDialog(None, u'Movimentação Contábil Mensal salva com sucesso!', 'Info', wx.OK)
                self.message.ShowModal()
                self.insereInCtrList(None)
    ...
    def vizualizaMovConIni(self, event, idMovCon):
        ...
        self.movConIni = MovConMensal.query.filter_by(id=idMovCon).first()

        self.windowEditaMovConIni = wx.MiniFrame(parent=self, id=wx.ID_ANY, size=(300, 280), pos=(300, 170), title=u'Editar - Movimetação Contábil Mensal', style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
    ...
    def insereContaPorCompetencia(self, event):
        ...
        if self.competenciaAtual == unicode(self.cbGeraArquivoCompetencia.GetValue()):
            ...
            movimentos = MovConMensal.query.filter_by(competencia=self.cbGeraArquivoCompetencia.GetValue()).all()
        else:

            movimentos = MovConMensal.query.all()
    ...
    def geraArquivoDialog(self, event):
        ...
        if self.movConIniParaArquivoListCtrl.GetItemCount() == 0:
            ...
            dlg = wx.FileDialog(self, message=u"Salvar ", defaultDir="", defaultFile="MOVCONMENSAL.REM", wildcard="Arquivo de Remessa (*.REM)|*.REM", style=wx.SAVE)
    ...
    def importaMovConMensal(self,event):
        ...
        for row_index in range(4,sheet.nrows-6):
            ...
            if contaExiste == None:
                ...
                MovConMensal(anoConta=unicode(datetime.datetime.now().year),
                    ...
                    credito=unicode(sheet.cell(row_index,4).value),
