...
class WindowMovConInicial(wx.MiniFrame):
    


    def __init__(self, parent):

        wx.MiniFrame.__init__(self, parent, id=wx.ID_ANY, size=(530, 340), pos=(300, 170), title=u"Movimentação Contábil Inicial", style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
        ...
        self.toolBar.AddLabelTool(ID_TOOLBAR_MOVCONINICIAL_NOVO, "Novo", wx.Bitmap("./imagens/add.png"), shortHelp=u'Adiciona novo Movimento Contábil Inicial')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_MOVCONINICIAL_EDITAR, "Editar", wx.Bitmap("./imagens/edit.png"), shortHelp=u'Edita Movimento Contábil Inicial selecionado')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_MOVCONINICIAL_EXCLUIR, "Remover", wx.Bitmap("./imagens/remove.png"), shortHelp=u'Exclui Movimento Contábil Inicial selecionado')
        self.toolBar.AddSeparator()
        self.toolBar.AddLabelTool(ID_TOOLBAR_MOVCONINICIAL_CRIAR_ARQUIVO, "Gerar Arquivo", wx.Bitmap("./imagens/file.png"), shortHelp=u'Gera arquivo de Movimento Contábil Inicial')
        self.toolBar.AddSeparator()
        self.toolBar.AddSeparator()
        ...
        self.Bind(wx.EVT_MENU, self.importaMovConIni, id=ID_TOOLBAR_MOVCONINICIAL_IMPORTAR)
    ...
    def toolBarControler(self, novo=True, editar=True, remover=True, gerar=True, importar=True):
        ...
        self.toolBar.EnableTool(ID_TOOLBAR_MOVCONINICIAL_CRIAR_ARQUIVO, gerar)
        self.toolBar.EnableTool(ID_TOOLBAR_MOVCONINICIAL_IMPORTAR, importar)
    ...
    def insereInCtrList(self, event):
        ...
        movimentos = MovConIni.query.filter_by(competencia=self.cbCompetenciaForView.GetValue()).all()
    ...
    def novoMovConIni(self, event):
        ...
        self.toolBarControler(False, False, False, False, False)

        self.windowNovoMovConIni = wx.MiniFrame(parent=self, id=wx.ID_ANY, size=(300, 280), pos=(300, 170), title=u'Novo - Movimetação Contábil Inicial', style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
    ...
    def salvarMovConIni(self, event):
        ...
        if self.valida():
            ...
            try:
                ...
                session.commit()
                self.message = wx.MessageDialog(None, u'Movimentação Contábil Inicial salva com sucesso!', 'Info', wx.OK)
                self.message.ShowModal()
                self.insereInCtrList(None)
    ...
    def vizualizaMovConIni(self, event, idMovCon):
        ...
        self.movConIni = MovConIni.query.filter_by(id=idMovCon).first()

        self.windowEditaMovConIni = wx.MiniFrame(parent=self, id=wx.ID_ANY, size=(300, 280), pos=(300, 170), title=u'Editar - Movimetação Contábil Inicial', style= wx.SYSTEM_MENU | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
    ...
    def editarMovConIni(self, event):
        ...
        if self.valida():
            ...
            self.movConIni.tipoMovimento = unicode(1)
    ...
    def insereContaPorCompetencia(self, event):
        ...
        if self.competenciaAtual == unicode(self.cbGeraArquivoCompetencia.GetValue()):
            ...
            movimentos = MovConIni.query.filter_by(competencia=self.cbGeraArquivoCompetencia.GetValue()).all()
        else:

            movimentos = MovConIni.query.all()
    ...
    def geraArquivoDialog(self, event):
        ...
        if self.movConIniParaArquivoListCtrl.GetItemCount() == 0:
            ...
            dlg = wx.FileDialog(self, message=u"Salvar ", defaultDir="", defaultFile="MOVCONINICIAL.REM", wildcard="Arquivo de Remessa (*.REM)|*.REM", style=wx.SAVE)
    ...
    def geraArquivo(self):
        ...
        for x in range(self.movConIniParaArquivoListCtrl.GetItemCount()):
            ...
            try:
                ...
                movConIni = MovConIni.query.filter_by(id=idConta).first()
    ...
    def importaMovConIni(self,event):
            ...
            if contaExiste == None:
                ...
                if sheet.cell(row_index,0).value[0] == '1':
                    ...
                    MovConIni(anoConta=unicode(year),
                        ...
                        tipoMovimento=unicode("1"),
                        debito=unicode(0),
                        credito=unicode(sheet.cell(row_index,2).value),
                        competencia=unicode(self.getMounthOnSheet(sheet))
                    )
                    session.commit()
                    contasInseridas +=1
